...
--
-- Name: check_username_conflict(); Type: FUNCTION; Schema: public; Owner: neondb_owner
--

CREATE FUNCTION public.check_username_conflict() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
ALTER FUNCTION public.check_username_conflict() OWNER TO neondb_owner;

--
-- Name: check_username_unique(); Type: FUNCTION; Schema: public; Owner: neondb_owner
--
...
ALTER FUNCTION public.check_username_unique() OWNER TO neondb_owner;

--
-- Name: check_username_uniqueness(); Type: FUNCTION; Schema: public; Owner: neondb_owner
--

CREATE FUNCTION public.check_username_uniqueness() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
  IF EXISTS (
...
ALTER FUNCTION public.check_username_uniqueness() OWNER TO neondb_owner;

--
-- Name: ensure_username_uniqueness(); Type: FUNCTION; Schema: public; Owner: neondb_owner
--

CREATE FUNCTION public.ensure_username_uniqueness() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM users 
    WHERE LOWER(username::text) = LOWER(NEW.username::text) 
    AND id != NEW.id
  ) THEN
    RAISE EXCEPTION 'Username already exists';
  END IF;
...
ALTER FUNCTION public.ensure_username_uniqueness() OWNER TO neondb_owner;
...
CREATE TABLE public.users (
    ...
    created_at timestamp without time zone DEFAULT now() NOT NULL,
    CONSTRAINT username_format_check CHECK ((username OPERATOR(public.~) '^[a-zA-Z0-9_-]{3,50}$'::public.citext))
);
...
6	user_6	38830c47a33eca5ed37ef5cf8a50e2c909b641543d5808085f13ec178a8255f1d3e6aad186c4d6564bf03b5e6de236b55341e1f1192ca869bcc69a0a651948aa.9a026069349e2779058086c6082d45b6	jicamet@gmail.com	\N	\N	es	user	2024-12-04 10:55:42.171379
\.
...
SELECT pg_catalog.setval('public.users_id_seq1', 14, true);
...
--
-- Name: users_username_citext_idx; Type: INDEX; Schema: public; Owner: neondb_owner
--

CREATE UNIQUE INDEX users_username_citext_idx ON public.users USING btree (username);
...
--
-- Name: users_username_unique_idx; Type: INDEX; Schema: public; Owner: neondb_owner
--
...
--
-- Name: users ensure_username_case_insensitive_unique; Type: TRIGGER; Schema: public; Owner: neondb_owner
--

CREATE TRIGGER ensure_username_case_insensitive_unique BEFORE INSERT OR UPDATE ON public.users FOR EACH ROW EXECUTE FUNCTION public.check_username_unique();


--
-- Name: users ensure_username_uniqueness; Type: TRIGGER; Schema: public; Owner: neondb_owner
--

CREATE TRIGGER ensure_username_uniqueness BEFORE INSERT OR UPDATE ON public.users FOR EACH ROW EXECUTE FUNCTION public.ensure_username_uniqueness();


--
-- Name: users username_conflict_check; Type: TRIGGER; Schema: public; Owner: neondb_owner
--

CREATE TRIGGER username_conflict_check BEFORE INSERT OR UPDATE ON public.users FOR EACH ROW EXECUTE FUNCTION public.check_username_conflict();
